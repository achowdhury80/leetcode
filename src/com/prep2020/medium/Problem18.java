...
public class Problem18 {
	public List<List<Integer>> fourSum(int[] nums, int target) {
	    ...
	    for(int i = 0; i < n - 3; i++){
	      int z = nums[i];
	      if (i > 0 && nums[i - 1] == z || (0l + z + 3l * max) < target) continue;
	      if(4l * z > target) break;
	      if(4l * z == target){
	  ...
	  private List<List<Integer>> threeSumForFour(int[] nums, int start, int target, int x){
	    ...
	    for(int i = start; i < n - 2; i++){
	      int z = nums[i];
	      if (i > start && nums[i - 1] == z || (0l + z + 2l * max) < target) continue;
	      if(3l * z > target) break;
	      if(3l * z == target){
	  ...
	  public static void main(String[] args) {
		  ...
		  System.out.println(problem.fourSum(new int[] {0,0,0,1000000000,1000000000,
